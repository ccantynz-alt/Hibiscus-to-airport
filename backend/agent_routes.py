...
def require_admin_token(x_admin_token: Optional[str]) -> None:
    ...
    if not expected:
        # Token not configured => allow (safe for dev). Recommended to set ADMIN_TOKEN in Render.
        return
...
def agent_cockpit():
    # Serve static HTML from file to avoid Python triple-quote escaping issues.
