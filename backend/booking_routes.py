...
async def create_booking(booking: BookingCreate):
    try:
        ...
        try:
            ...
            if is_urgent:
                logger.warning(f"ðŸš¨ URGENT BOOKING DETECTED: {booking_ref} - only {hours_until}hrs notice!")
                try:
...
Questions? 021 743 321"""
        ...
        return {
            ...
            "sms_sent": sms_sent,
            "message": f"Payment link sent! Email: {'âœ“' if email_sent else 'âœ—'}, SMS: {'âœ“' if sms_sent else 'âœ—'}"
        }
...
async def send_driver_job_notification(booking: dict, driver: dict, payout: float, token: str, notes: str = ""):
    ...
    try:
        ...
        if driver_email:
            subject = f"ðŸš— NEW JOB: {booking_ref} - ${payout:.2f}"
            ...
            <div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif;">
              <div style="background: linear-gradient(135deg, #1f2937 0%, #111827 100%); color: white; padding: 30px; border-radius: 10px 10px 0 0;">
                <h1 style="margin: 0; font-size: 24px;">ðŸš— New Job Available</h1>
              ...
              <div style="background: white; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e5e7eb;">
                ...
                <div style="background: #f8fafc; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #f59e0b;">
                  <p style="margin: 0;"><strong>ðŸ“… Date:</strong> {booking_date}</p>
                  <p style="margin: 10px 0 0;"><strong>â° Pickup Time:</strong> {booking_time}</p>
                  <p style="margin: 10px 0 0;"><strong>ðŸ“ Pickup:</strong> {booking.get('pickupAddress', 'N/A')}</p>
                  <p style="margin: 10px 0 0;"><strong>ðŸ Drop-off:</strong> {booking.get('dropoffAddress', 'N/A')}</p>
                  <p style="margin: 10px 0 0;"><strong>ðŸ‘¥ Passengers:</strong> {booking.get('passengers', 1)}</p>
                  <p style="margin: 10px 0 0;"><strong>ðŸ‘¤ Customer:</strong> {booking.get('name', 'N/A')}</p>
                </div>
                
                {f'<div style="background: #e0f2fe; padding: 15px; border-radius: 8px; margin: 20px 0;"><p style="margin: 0; font-size: 14px; color: #0369a1;"><strong>ðŸ“ Notes:</strong> {notes}</p></div>' if notes else ''}
...
async def driver_respond_to_job(booking_id: str, data: dict):
    ...
    try:
        ...
        if accepted:
            ...
            send_email(
                admin_email,
                f"âœ… Driver ACCEPTED: {booking.get('booking_ref')}",
            ...
            send_email(
                admin_email,
                f"âŒ Driver DECLINED: {booking.get('booking_ref')}",
...
async def send_5min_arrival_sms(tracking_session: dict, tracking_id: str):
    ...
    try:
        ...
        message = f"Hi {customer_name.split()[0]}! ðŸš— Your driver {driver_name} is approximately 10 minutes away. Track live: {tracking_url}"
...
def fallback_response(session: WhatsAppSession, user_message: str) -> str:
    ...
    if session.state == "greeting":
        return "Hi! ðŸ‘‹ Welcome to Hibiscus to Airport! Where would you like to be picked up from?\n\n[NONE]"
...
async def whatsapp_webhook(
    ...
    try:
        ...
        if message.lower() in ['reset', 'start over', 'cancel', 'restart']:
            reset_session(phone)
            session = get_or_create_session(phone)
            response_text = "No problem! Let's start fresh. ðŸ”„\n\nWhere would you like to be picked up from?"
            ...
            if extracted['type'] == 'reset':
                ...
                try:
                    ...
                    response_text += f"\n\nðŸ’° **Your Quote:**\n"
                    response_text += f"ðŸ“ From: {session.pickup_address}\n"
                    response_text += f"ðŸ“ To: {session.dropoff_address}\n"
                    response_text += f"ðŸ“… Date: {session.date}\n"
                    response_text += f"â° Time: {session.time}\n"
                    response_text += f"ðŸ‘¥ Passengers: {session.passengers}\n"
                    response_text += f"ðŸ’µ **Total: ${session.pricing['totalPrice']:.2f} NZD**\n\n"
                ...
                if session.state == "confirming" and session.pricing:
                    ...
                    try:
                        ...
                        response_text += f"ðŸ“‹ Reference: **{booking_ref}**\n"
                        response_text += f"ðŸ’µ Total: **${session.pricing['totalPrice']:.2f} NZD**\n\n"
                        response_text += f"ðŸ’³ Pay securely here:\n{payment_url}\n\n"
                        response_text += "Or pay cash to the driver on pickup day.\n\n"
                        response_text += "Questions? Just message us here! ðŸ˜Š"
...
ðŸ‘¤ Customer: {booking.get('name', 'N/A')}
ðŸ“ž Phone: {booking.get('phone', 'N/A')}
âœ‰ï¸ Email: {booking.get('email', 'N/A')}

ðŸ“ Pickup: {booking.get('pickupAddress', 'N/A')}
ðŸ Drop-off: {booking.get('dropoffAddress', 'N/A')}

ðŸ‘¥ Passengers: {booking.get('passengers', 1)}
ðŸ’° Total: ${booking.get('pricing', {}).get('totalPrice', 0):.2f} NZD
...
ðŸ“ Notes: {booking.get('notes', 'None')}
...
""".strip()
        ...
        event = {
            'summary': f"ðŸš— {booking_ref} - {booking.get('name', 'Customer')} | {booking.get('passengers', 1)} pax",
...
async def create_test_calendar_event(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        event = {
            'summary': 'ðŸ§ª Test Booking - Hibiscus to Airport',
          ...
          <div style="background: white; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e5e7eb;">
            ...
            <div style="background: #fef3c7; padding: 15px; border-radius: 8px; margin: 20px 0;">
              <p style="margin: 0; font-size: 14px; color: #92400e;">
                <strong>ðŸ“Œ Please be ready 5-10 minutes before your pickup time.</strong><br>
