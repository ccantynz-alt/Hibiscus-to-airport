...
function Ensure-AdminProxy {
  ...
  if ($isNext) {
    ...
    $base = (Test-Path -LiteralPath ".\src\pages") ? ".\src\pages" : ".\pages"
    $ext = $useTs ? "ts" : "js"
...
while ($true) {
  ...
  if ($bClass -ne "BACKEND_OK") {
    Warn "Backend unhealthy (this loop doesn't control Render deploy without credentials)."
  } else {
    Ok "Healthy enough."
  }
