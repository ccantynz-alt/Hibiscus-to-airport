...
const AdminLogin = () => {
  ...
  const handleKeyLogin = async () => {
    ...
    setAuthError('');
    try {
      const response = await axios.post(`${BACKEND_URL}/api/admin/key-login`, { key: adminKey });
      localStorage.setItem('admin_token', response.data.access_token);
      toast({ title: 'Login Successful!', description: 'Signed in with admin key' });
      navigate('/admin/dashboard');
    } catch (error) {
      const message = error.response?.data?.detail || 'Invalid admin key';
    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="bg-gray-900/80 backdrop-blur-md border-2 border-gold/20 rounded-2xl p-8 shadow-2xl">
          ...
          {/* Emergency Admin Key Login (no third-party OAuth) */}
          <div className="space-y-3 mb-6">
            <label className="block text-sm font-medium text-gray-300">Emergency Admin Key</label>
            <Input
              type="password"
              value={adminKey}
            ...
            <Button
              ...
              className="w-full bg-white hover:bg-gray-100 text-gray-900 font-bold py-6"
            >
              {keyLoading ? (
                <>
                  <Loader2 className="w-4 h-4 mr-2 animate-spin" />
            ...
            <p className="text-xs text-gray-400">
              If you do not know this key, set <span className="text-gray-200">ADMIN_API_KEY</span> in your backend hosting
              (Render) and redeploy.
            </p>
          </div>
